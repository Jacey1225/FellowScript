...
class AgentManager(DBManager):
    ...
    def note_via_hb(self, data: dict, group_id: Optional[str] = None) -> str:
    ...
    def commit_hb_response(self, agent_id: str, heartbeat_id: str, heartbeat_content: str, force: bool = False):
        ...
        # The heartbeat row's own group_id -- source of truth for the note
        # this fire will generate (see note_via_hb's docstring). Read once
        # here so both the forced and unforced paths below thread the same
        # value through rather than either one trusting the LLM's response.
        heartbeat_group_id = list(owned.values())[0].get("group_id") or None

        if force:
            return self._commit_hb_response_forced(agent_id, heartbeat_id, heartbeat_content, heartbeat_group_id)
