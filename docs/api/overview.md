...
A second, separate admin panel (task `20260918-admin-activity-monitoring`) from the CloudWatch console above — average per-user activity and website-visit trends, rendered server-side as matplotlib PNGs. One endpoint is intentionally public; the rest are **admin-only** (`require_admin`, same `401`/`403` semantics as Monitoring above).
