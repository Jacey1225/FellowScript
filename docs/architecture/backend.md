...
| File | Responsibility |
|---|---|
| `subscriptions.py` | `SubscriptionsManager` — create free plan, get subscription, reconcile expired, grant admin comp membership |
...
There is also a separate, admin-only `plan_type='individual'` row shape (`provider='admin_comp'`), unrelated to the old pre-`GROUP_PRICE_CENTS` "Individual" tier described above — it's a $0, non-billed grant created via `POST /subscriptions/admin/grant-individual` (`require_admin`) for the app's own admin account(s) to get free unlimited access without going through Stripe/Apple. See `SubscriptionsManager.grant_admin_comp` and the API reference.
...
| `_advance_recurring_sessions` | every `SESSION_RECURRING_ADVANCE_POLL_INTERVAL_SECONDS` (60s, 2026-09-21) | Scans `devotions` for any `recurring = TRUE` session whose `time_end` is more than `SESSION_RECURRING_ADVANCE_GRACE_SECONDS` (1h) in the past, gates on the same fail-closed AWS Chime presence check the auto-delete job uses, then advances `time_start`/`time_end` in place by exactly one calendar week — computed in the session creator's own local timezone (`users.timezone`, zoneinfo-aware, so a DST transition doesn't shift the wall-clock time) rather than a raw 7-day UTC offset. The same atomic UPDATE also resets `reminder_sent_at` to `NULL` (so the advanced occurrence's start-time push re-arms) and `chime_meeting_id`/`chime_meeting` to their column defaults (`''`/`{}`, so the new occurrence never inherits a finished call's meeting state), re-checking the row's `time_end` at write time to avoid a double-advance across concurrent poll cycles. A non-recurring session is never touched by this job; a malformed `time_end` or an unresolvable creator timezone is a fail-closed skip, not a guess. No push/websocket notification of the new time is sent — clients pick it up on their next session fetch. |
