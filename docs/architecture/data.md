...
| `title` | TEXT | |
| `participants` | UUID[] | |
| `time_start` / `time_end` | TIMESTAMPTZ | Absolute scheduled window, set via the session-creator UI. For a `recurring = TRUE` session, these are advanced in place by exactly one calendar week (computed in the creator's local timezone, DST-aware) once the current occurrence's `time_end` has passed — see `_advance_recurring_sessions` (2026-09-21) in [Background Scheduler](backend.md#background-scheduler). A `recurring = FALSE` session's window is never touched by that job. |
| `recurring` | BOOLEAN DEFAULT FALSE | The session-creator UI's "Repeat weekly" chip. Drives `_advance_recurring_sessions`' weekly roll-forward (2026-09-21) and is what makes that job's candidate set and `_auto_delete_expired_sessions`' candidate set mutually exclusive on this same table. |
| `reminder_sent_at` | TIMESTAMPTZ, nullable (2026-09-04) | Atomic one-shot claim marker for the `time_start` reminder push — `_fire_due_session_reminders` (see [Background Scheduler](backend.md#background-scheduler)) claims a session via `UPDATE ... WHERE reminder_sent_at IS NULL`, so a session is reminded exactly once regardless of poll cadence or concurrent pollers. Not a per-day/rolling window like the heartbeat/midday markers — a session's reminder is a one-shot event. For a `recurring = TRUE` session, `_advance_recurring_sessions` resets this to `NULL` on every weekly advance (2026-09-21), so each new occurrence re-arms its own one-shot reminder rather than staying permanently claimed. |
| `summarize` | BOOLEAN DEFAULT FALSE (2026-09-07) | Set from the session-creation sheet's "Summarize" toggle; deny-by-default per Security Posture Q2. When true, the iOS client calls [`POST /agent/{user_id}/{agent_id}/summarize`](../api/overview.md#agent-ai-check-ins) as soon as the session's Chime call ends, saving an LLM-generated note for the user — see that endpoint's entry for the full client-side wiring (agent resolution, creator-only trigger, non-blocking failure handling). Unlike `reminder_sent_at`/`chime_meeting_id`, this is a persistent user preference, not per-occurrence state — `_advance_recurring_sessions` leaves it untouched on a weekly advance. |
| `chime_meeting_id` / `chime_meeting` | VARCHAR DEFAULT `''` / JSONB DEFAULT `'{}'` | The session's live Chime call state. `_advance_recurring_sessions` resets both to their column defaults on every weekly advance (2026-09-21), so a new occurrence never inherits a finished call's meeting state; the advance itself is gated on the same fail-closed AWS-confirmed-empty presence check `_auto_delete_expired_sessions` uses, so a session is never advanced (and its meeting state cleared) out from under an actively-occupied call. |
...
| `agent_heartbeats` | `_id`, `user_id FK→users ON DELETE CASCADE`, `last_fired TIMESTAMPTZ`, `group_id FK→groups ON DELETE SET NULL` (nullable — 2026-09-02), `timeline_instruction TEXT` (nullable — 2026-09-06) |

`agent_heartbeats.timeline_instruction` (2026-09-06) replaced the old `agentic_context` table + `AgentManager.save_context`/`get_context` reactive dedup mechanism entirely — that table and both methods are gone. Instead, each heartbeat carries a single JSON-encoded upfront content plan for its current up-to-31-day window: `{"window_start": "<ISO date>", "days": {"<offset>": "<instruction>", ...}, "coverage_summary": "<rolling digest>"}`. `days` is keyed by window-offset (0–30, calendar days since `window_start`), one entry per offset the heartbeat's `timestamps` schedule actually fires on. A dedicated timeline-planning LLM call produces this plan once, synchronously, when the heartbeat is created (`AgentManager.add_heartbeat` — the heartbeat row is never created without one); it's regenerated lazily by `AgentManager.ensure_current_timeline` the next time the heartbeat fires after its window has elapsed, after a schedule (`timestamps`) edit nulls the column out (`update_heartbeat`), or for a heartbeat that predates this feature. Regeneration is informed by the prior plan's `coverage_summary` so a new window's content doesn't repeat what earlier windows already covered, without needing to keep a full history table. `ensure_current_timeline` only ever runs after a fire has already won its exclusive claim (the normal path's `last_fired` UPDATE, or the forced path's advisory lock), so two racing fires for the same heartbeat can never both regenerate it. At fire time, `AgentManager._generate_and_save_note` reads that day's specific instruction straight off this column instead of the old CHAPTERS/VERSES/THEME aggregate.
