...
- **Keyboard dismiss on scroll or tap** — `View.dismissesKeyboardOnScrollAndTap()` composes `.scrollDismissesKeyboard(.interactively)` with a `simultaneousGesture` tap recognizer that resigns first responder app-wide. Applied to every screen with text input (Auth, Account, Chat composers/search/sheets, Notes editor/reply sheet, MFA/report/event sheets). Reconciles `NoteEditorView`'s prior one-off `.scrollDismissesKeyboard(.interactively)` onto this same shared modifier.
