...
| `Dashboard/DashboardView.swift`, `Dashboard/DashboardComponents.swift` | The home tab's "Editorial Hero" layout, and its *entire* content: a single greeting header (time-of-day + live username, no eyebrow/subtitle text), a Friend Activity hero card (avatar stack of active friends, the most-recently-active friend's headline/timestamp — tap to open their chat — and, when they have one, a preview of their most recent public note), a check-in nudge row (the friend gone longest without a direct message), and a note-resume card (the user's own most recent note). Sourced from `GET /friends/{user_id}/activity` (`FriendsManager.get_friend_activity`) plus the view's own notes fetch, both cache-first via `DiskCache`. Replaces the former `GroupActivityWidget`. The pre-existing `RevisitVerseCard`, `BookmarksWidget`, the notes/highlights stat cards, `MyGroupsRow`, and `QuickActionsRow` (and their backing `DashboardViewModel` state/fetches) have been removed outright, not just demoted below the hero — the Editorial Hero content is the whole dashboard now, matching the approved mockup exactly (no leaked internal labels like "YOUR RHYTHM"/"FRIEND ACTIVITY"/"PICK UP WHERE YOU LEFT OFF" either). `glassCard()` still carries its optional `blurBoost` parameter (default `0`, no behavior change for other callers) — only the hero and note-resume cards pass it, for the more-transparent/heavier-blur glass treatment ported from the "Editorial Hero" mockup. |
