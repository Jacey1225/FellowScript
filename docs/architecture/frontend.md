...
| `Dashboard/DashboardView.swift`, `Dashboard/DashboardComponents.swift` | The home tab's "Editorial Hero" layout: a Friend Activity hero card (avatar stack of active friends, the most-recently-active friend's headline/timestamp — tap to open their chat — and, when they have one, a preview of their most recent public note), a check-in nudge row (the friend gone longest without a direct message), and a note-resume card (the user's own most recent note). Sourced from the new `GET /friends/{user_id}/activity` endpoint (`FriendsManager.get_friend_activity`) alongside the view's existing per-resource fetches (notes/highlights/bookmarks/agents/contacts), all cache-first via `DiskCache`. Replaces the former `GroupActivityWidget`; its "Continue reading" affordance lives on unchanged in `QuickActionsRow`'s "Read" quick action. `RevisitVerseCard`, `BookmarksWidget`, the notes/highlights stat cards, `MyGroupsRow`, and `QuickActionsRow` are demoted below the new hero section rather than removed. `glassCard()` gained an optional `blurBoost` parameter (default `0`, no behavior change for existing callers) — only the new hero and note-resume cards pass it, for the more-transparent/heavier-blur glass treatment ported from the "Editorial Hero" mockup. |
