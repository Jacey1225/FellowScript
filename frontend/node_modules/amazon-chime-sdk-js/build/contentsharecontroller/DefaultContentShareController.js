...
const ContentShareConstants_1 = __importDefault(require("./ContentShareConstants"));
class DefaultContentShareController {
    static createContentShareMeetingSessionConfigure(configuration) {
        ...
        contentShareConfiguration.videoDownlinkBandwidthPolicy = new NoVideoDownlinkBandwidthPolicy_1.default();
    ...
    enableSimulcastForContentShare(enable, encodingParams) {
        if (enable) {
            ...
            this.contentAudioVideo.configuration.videoUplinkBandwidthPolicy =
                new DefaultSimulcastUplinkPolicyForContentShare_1.default(this.contentAudioVideo.logger, encodingParams);
        }
        else {
    ...
    enableSVCForContentShare(enable) {
        if (enable) {
    ...
    async startContentShare(stream) {
        if (!stream) {
            return;
        }
        if (this.contentAudioVideo.configuration.meetingFeatures.contentMaxResolution ===
            VideoQualitySettings_1.default.VideoDisabled) {
            this.contentAudioVideo.logger.info('Could not start content because max content resolution was set to None');
            return;
        }
    ...
    async startContentShareFromScreenCapture(sourceId, frameRate) {
