...
const AllHighestVideoBandwidthPolicy_1 = __importDefault(require("./AllHighestVideoBandwidthPolicy"));
...
class VideoPriorityBasedPolicy {
    constructor(logger, videoPriorityBasedPolicyConfig = VideoPriorityBasedPolicyConfig_1.default.Default) {
        ...
        if (this.videoPriorityBasedPolicyConfig.serverSideNetworkAdaption ===
            ServerSideNetworkAdaption_1.default.Default) {
    ...
    reset() {
        ...
        this.rateProbeState = "Not Probing" /* RateProbeState.NotProbing */;
    ...
    chooseRemoteVideoSources(preferences) {
        ...
        if (!(0, ServerSideNetworkAdaption_1.serverSideNetworkAdaptionIsNoneOrDefault)(this.serverSideNetworkAdaption)) {
    ...
    calculateOptimalReceiveStreams() {
        ...
        const remoteInfosWithPreference = remoteInfos.filter((stream) => {
        ...
        for (const preference of this.getCurrentVideoPreferences()) {
            const streamsForAttendees = remoteInfosWithPreference.filter((description) => {
            ...
            streamsForAttendees.forEach((stream) => {
                maxFrameRate = Math.max(maxFrameRate, stream.maxFrameRate);
                maxPixels = Math.max(maxPixels, stream.totalPixels());
            });
            streamsForAttendees.sort((a, b) => {
                if (preference.degradationPreference === VideoQualityAdaptationPreference_1.default.MaintainResolution) {
                ...
                else if (preference.degradationPreference === VideoQualityAdaptationPreference_1.default.MaintainFramerate) {
                    ...
                    const weighByFramerate = (stream) => {
                        return (stream.avgBitrateKbps *
                            Math.pow(2, ((framerateExponentConstant * stream.maxFrameRate) / maxFrameRate) *
            ...
            for (const info of streamsForAttendees) {
                if (lastInfo !== undefined) {
                    if ((info.maxFrameRate < lastInfo.maxFrameRate &&
        ...
        const filteredRemoteAttendeesWithPreference = remoteInfosWithPreference.filter((info) => {
        ...
        const upgradeStream = this.priorityPolicy(rates, filteredRemoteAttendeesWithPreference, chosenStreams);
        const skipProbe = !(0, ServerSideNetworkAdaption_1.serverSideNetworkAdaptionIsNoneOrDefault)(this.serverSideNetworkAdaption);
        let subscriptionChoice = 0 /* UseReceiveSet.NewOptimal */;
        // Look for probing or override opportunities
        if (!skipProbe && !this.startupPeriod && sameStreamChoices) {
            if (this.rateProbeState === "Probing" /* RateProbeState.Probing */) {
            ...
            this.setProbeState("Not Probing" /* RateProbeState.NotProbing */);
            this.lastUpgradeRateKbps = 0;
        }
        this.previousStreamsWithPreference = remoteInfosWithPreference;
        this.videoPreferencesUpdated = false;
        if (subscriptionChoice === 1 /* UseReceiveSet.PreviousOptimal */) {
        ...
        if (subscriptionChoice === 2 /* UseReceiveSet.PreProbe */) {
    ...
    setProbeState(newState) {
        ...
        const now = Date.now();
        switch (newState) {
            case "Not Probing" /* RateProbeState.NotProbing */:
                this.probePendingStartTimestamp = 0;
                break;
            case "Probe Pending" /* RateProbeState.ProbePending */:
                ...
                break;
            case "Probing" /* RateProbeState.Probing */:
    ...
    handleProbe(chosenStreams, targetDownlinkBitrate) {
        ...
        if (Date.now() - this.lastProbeTimestamp > VideoPriorityBasedPolicy.MAX_ALLOWED_PROBE_TIME_MS) {
            ...
            this.setProbeState("Not Probing" /* RateProbeState.NotProbing */);
            return 0 /* UseReceiveSet.NewOptimal */;
        }
        if (this.downlinkStats.packetsLost > 0) {
            ...
            if (this.downlinkStats.packetsLost > VideoPriorityBasedPolicy.SPURIOUS_PACKET_LOST_THRESHOLD) {
                ...
                return 2 /* UseReceiveSet.PreProbe */;
            }
        }
        const subscribedRate = this.calculateSubscribeRate(this.optimalReceiveStreams);
    ...
    maybeOverrideOrProbe(chosenStreams, rates, upgradeStream) {
        ...
        if (!sameSubscriptions &&
            ...
            this.logger.info('bwe: MaybeOverrideOrProbe: Reuse last decision based on delta rate. {' +
                ...
                `}`);
            useLastSubscriptions = 1 /* UseReceiveSet.PreviousOptimal */;
        }
        else {
        ...
        if (sameSubscriptions || useLastSubscriptions === 1 /* UseReceiveSet.PreviousOptimal */) {
            ...
            switch (this.rateProbeState) {
                case "Not Probing" /* RateProbeState.NotProbing */:
                    this.setProbeState("Probe Pending" /* RateProbeState.ProbePending */);
                    break;
                case "Probe Pending" /* RateProbeState.ProbePending */:
                    if (this.setProbeState("Probing" /* RateProbeState.Probing */)) {
                        this.upgradeToStream(chosenStreams, upgradeStream);
                        useLastSubscriptions = 0 /* UseReceiveSet.NewOptimal */;
                    }
                    break;
            }
        }
        ...
        while (priority !== -1) {
            ...
            for (const preference of videoPreferences) {
                if (preference.priority === priority) {
                    for (const info of remoteInfos) {
                        if (info.attendeeId === preference.attendeeId) {
                            const index = chosenStreams.findIndex(stream => stream.groupId === info.groupId &&
    ...
    canUpgrade(bitrateKbps, targetResolution, targetBitrateKbps, isContent) {
        ...
        if (targetResolution === TargetDisplaySize_1.default.High) {
            ...
            targetResolution === TargetDisplaySize_1.default.Medium &&
            bitrateKbps <= targetBitrateKbps) {
            // If the target resolution is medium then fall back to use avg bitrate as video.
            canUpgrade = true;
        }
        if (canUpgrade) {
            this.logger.info(`bwe: canUpgrade: bitrateKbps: ${bitrateKbps} targetBitrateKbps: ${targetBitrateKbps}`);
            return true;
        }
        this.logger.info(`bwe: cannot Upgrade: bitrateKbps: ${bitrateKbps} targetBitrateKbps: ${targetBitrateKbps}`);
        return false;
    }
    ...
    streamsWithPreferenceDidChange(remoteInfos) {
        if (this.previousStreamsWithPreference === undefined ||
            remoteInfos.length !== this.previousStreamsWithPreference.length ||
            this.videoPreferencesUpdated === true) {
            return true;
        }
        for (const info of remoteInfos) {
            const infoMatch = this.previousStreamsWithPreference.find(prevInfo => prevInfo.groupId === info.groupId &&
    ...
    setServerSideNetworkAdaption(adaption) {
        ...
        this.setProbeState("Not Probing" /* RateProbeState.NotProbing */); // In case we were probing
    }
