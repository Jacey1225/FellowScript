...
class VideoFxProcessor {
     ...
     * @param stream frame from current video stream about to be processed
     */
    async adjustProcessorForNewStreamParameters(stream) {
        ...
        await this.renderer.configure(this.streamParameters.width, this.streamParameters.height, this.effectConfig);
    }
    /**
     ...
     */
    async manageSegmentationMask(inputCanvas) {
        try {
     ...
     */
    async generateSegmentationMask(inferenceImageData) {
        if (VideoFxProcessor.isSharedArrayBufferSupported) {
     ...
     */
    async setEffectConfig(effectConfig) {
        ...
        if (newEffectConfig.backgroundReplacement.isEnabled) {
            ...
            await this.renderer.setBackgroundReplacementCanvas(this.canvasOpsManager.getBackgroundReplacementCanvas());
        }
        // Configure the renderer to implement the desired effect configuration
        await this.renderer.configure(this.streamParameters.width, this.streamParameters.height, newEffectConfig);
     ...
     */
    async validateEffectConfig(config) {
     ...
     */
    async validateReplacementConfig(config) {
    ...
    getVideoFxAssetParams() {
        const defaultAssetSpec = (0, Utils_1.getDefaultAssetSpec)();
     ...
     */
    async loadEngineWorker(videoFxAssetParams) {
        ...
        try {
            ...
            this.engineWorker = await (0, loader_1.loadWorker)(engineWorkerPath, 'VideoFxEngineWorker', {}, null);
     ...
     */
    async buildEngine(videoFxAssetParams) {
     ...
     */
    async loadFxLib(videoFxAssetParams) {
        // Determine engine worker path from versioning of the SDK worker
        ...
        try {
            const res = await (0, fetch_1.fetchWithBehavior)(fxLibPath, WORKER_FETCH_OPTIONS, {});
            if (!res.ok) {
     ...
     */
    static async isSupported(logger = new NoOpLogger_1.default(), attemptAssetLoad = true) {
        ...
        checkEnv(!(0, support_1.supportsWorker)(globalThis, logger), 'Browser does not support web workers.');
        // Check wasm is supported
        checkEnv(!(0, support_1.supportsWASM)(globalThis, logger), 'Browser does not support wasm.');
     ...
     * AWS services.
     */
    static async create(logger, effectConfig, processingBudgetPerFrame = VideoFxConstants_1.RESOURCE_CONSTRAINTS.DEFAULT_PROCESSING_BUDGET_PER_FRAME, spec) {
