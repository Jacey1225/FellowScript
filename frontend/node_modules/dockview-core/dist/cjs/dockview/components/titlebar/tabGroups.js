...
var TabGroupManager = /** @class */ (function () {
    ...
    TabGroupManager.prototype.update = function () {
        ...
        try {
            ...
            for (var _d = __values(this._chipRenderers), _e = _d.next(); !_e.done; _e = _d.next()) {
                var _f = __read(_e.value, 2), groupId = _f[0], entry = _f[1];
                if (!activeGroupIds.has(groupId)) {
                    entry.chip.element.remove();
                    entry.chip.dispose();
                    (_c = entry.dragSourcesDisposable) === null || _c === void 0 ? void 0 : _c.dispose();
    ...
    TabGroupManager.prototype.updateDragAndDropState = function () {
        ...
        try {
            for (var _e = __values(this._chipRenderers.values()), _f = _e.next(); !_f.done; _f = _e.next()) {
                var entry = _f.value;
                entry.chip.element.draggable = caps.html5;
                (_b = entry.html5DragSource) === null || _b === void 0 ? void 0 : _b.setDisabled(!caps.html5);
                (_c = entry.pointerDragSource) === null || _c === void 0 ? void 0 : _c.setDisabled(!caps.pointer);
                (_d = entry.pointerDragSource) === null || _d === void 0 ? void 0 : _d.setTouchOnly(!caps.pointerHandlesMouse);
            }
        }
    ...
    TabGroupManager.prototype.disposeAll = function () {
        ...
        this._indicator = null;
        try {
            for (var _e = __values(this._pendingTransitionCleanups), _f = _e.next(); !_f.done; _f = _e.next()) {
                var _g = __read(_f.value, 2), cleanup = _g[1];
                cleanup();
            }
        }
        ...
        try {
            for (var _h = __values(this._chipRenderers), _j = _h.next(); !_j.done; _j = _h.next()) {
                var _k = __read(_j.value, 2), entry = _k[1];
                entry.chip.element.remove();
                entry.chip.dispose();
                (_d = entry.dragSourcesDisposable) === null || _d === void 0 ? void 0 : _d.dispose();
                entry.disposable.dispose();
            }
        }
     ...
     */
    TabGroupManager.prototype._createChipDragSources = function (chip, tabGroup) {
        var _this = this;
        var caps = (0, dndCapabilities_1.resolveDndCapabilities)(this._ctx.accessor.options);
    ...
    TabGroupManager.prototype._ensureChipForGroup = function (tabGroup) {
        var _this = this;
        var existing = this._chipRenderers.get(tabGroup.id);
        if (existing) {
            ...
            if (!existing.html5DragSource) {
                var sources = this._createChipDragSources(existing.chip, tabGroup);
        ...
        var dragSources = this._createChipDragSources(chip, tabGroup);
        var disposables = [
        ...
        var dropTarget = new droptarget_1.Droptarget(chip.element, {
            ...
            canDisplayOverlay: function (event, position) {
                ...
                if (_this._ctx.accessor.id === (data === null || data === void 0 ? void 0 : data.viewId)) {
