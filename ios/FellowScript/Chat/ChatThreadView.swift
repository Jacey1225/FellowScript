...
struct ChatThreadView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    // + new study session (mirrors ChatView "New Study Session" button)
        ...
        .onAppear { messages = MockDataService.mockMessages; sessions = [MockDataService.mockSession] }
        .sheet(isPresented: $showSession) {
            SessionCreatorSheet(groupId: contact.id, onSave: { _ in showSession = false })
        }
    }
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button("Schedule") {
                        let session = FSSession(
                            id: UUID().uuidString,
                            title: title,
                            time_start: ISO8601DateFormatter().string(from: startDate),
                            time_end: ISO8601DateFormatter().string(from: startDate.addingTimeInterval(Double(duration * 60))),
                            verses: verses, prompts: prompts,
                            recurring: recurring, summarize: summarize,
                            group_id: groupId
                        )
                        onSave(session)
                        dismiss()
